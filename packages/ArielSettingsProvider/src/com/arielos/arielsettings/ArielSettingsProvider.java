 ...
 * Copyright (C) 2022 The ArielOS Project
 *
...
package com.arielos.arielsettings;
...
/**
 * The ArielSettingsProvider serves as a {@link ContentProvider} for Ariel specific settings
 */
public class ArielSettingsProvider extends ContentProvider {
    ...
    protected final SparseArray<ArielDatabaseHelper> mDbHelpers = new SparseArray<ArielDatabaseHelper>();
    ...
    static {
        sUriMatcher.addURI(ArielSettings.AUTHORITY, ArielDatabaseHelper.ArielTableNames.TABLE_SYSTEM,
                SYSTEM);
        sUriMatcher.addURI(ArielSettings.AUTHORITY, ArielDatabaseHelper.ArielTableNames.TABLE_SECURE,
                SECURE);
        sUriMatcher.addURI(ArielSettings.AUTHORITY, ArielDatabaseHelper.ArielTableNames.TABLE_GLOBAL,
                GLOBAL);
        sUriMatcher.addURI(ArielSettings.AUTHORITY, ArielDatabaseHelper.ArielTableNames.TABLE_SYSTEM +
    ...
    public boolean onCreate() {
        if (LOCAL_LOGV) Log.d(TAG, "Creating ArielSettingsProvider");
        ...
        mUriBuilder.authority(ArielSettings.AUTHORITY);
        ...
        ServiceManager.addService("arielsettings", new ArielSettingsService(this));
    ...
    public Bundle call(String method, String request, Bundle args) {
        ...
        if (args != null) {
            int reqUser = args.getInt(ArielSettings.CALL_METHOD_USER_KEY, callingUserId);
        ...
        switch (method) {
            ...
            case ArielSettings.CALL_METHOD_GET_SYSTEM:
                return lookupSingleValue(callingUserId, ArielSettings.System.CONTENT_URI,
                        request);
            case ArielSettings.CALL_METHOD_GET_SECURE:
                return lookupSingleValue(callingUserId, ArielSettings.Secure.CONTENT_URI,
                        request);
            case ArielSettings.CALL_METHOD_GET_GLOBAL:
                return lookupSingleValue(callingUserId, ArielSettings.Global.CONTENT_URI,
                        request);

            // Put methods
            case ArielSettings.CALL_METHOD_PUT_SYSTEM:
                enforceWritePermission(arielos.platform.Manifest.permission.WRITE_SETTINGS);
                callHelperPut(callingUserId, ArielSettings.System.CONTENT_URI, request, args);
                return null;
            case ArielSettings.CALL_METHOD_PUT_SECURE:
                enforceWritePermission(
                        arielos.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                callHelperPut(callingUserId, ArielSettings.Secure.CONTENT_URI, request, args);
                return null;
            case ArielSettings.CALL_METHOD_PUT_GLOBAL:
                enforceWritePermission(
                        arielos.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                callHelperPut(callingUserId, ArielSettings.Global.CONTENT_URI, request, args);
                return null;

            // List methods
            case ArielSettings.CALL_METHOD_LIST_SYSTEM:
                return callHelperList(callingUserId, ArielSettings.System.CONTENT_URI);
            case ArielSettings.CALL_METHOD_LIST_SECURE:
                return callHelperList(callingUserId, ArielSettings.Secure.CONTENT_URI);
            case ArielSettings.CALL_METHOD_LIST_GLOBAL:
                return callHelperList(callingUserId, ArielSettings.Global.CONTENT_URI);

            // Delete methods
            case ArielSettings.CALL_METHOD_DELETE_SYSTEM:
                enforceWritePermission(arielos.platform.Manifest.permission.WRITE_SETTINGS);
                return callHelperDelete(callingUserId, ArielSettings.System.CONTENT_URI,
                        request);
            case ArielSettings.CALL_METHOD_DELETE_SECURE:
                enforceWritePermission(
                        arielos.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                return callHelperDelete(callingUserId, ArielSettings.Secure.CONTENT_URI,
                        request);
            case ArielSettings.CALL_METHOD_DELETE_GLOBAL:
                enforceWritePermission(
                        arielos.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                return callHelperDelete(callingUserId, ArielSettings.Global.CONTENT_URI,
                        request);
        }
    ...
    private Cursor queryForUser(int userId, Uri uri, String[] projection, String selection,
        ...
        ArielDatabaseHelper dbHelper = getOrEstablishDatabase(getUserIdForTable(tableName, userId));
    ...
    private Uri insertForUser(int userId, Uri uri, ContentValues values) {
        ...
        if (ArielDatabaseHelper.ArielTableNames.TABLE_SYSTEM.equals(tableName)) {
            validateSystemSettingNameValue(name, value);
        } else if (ArielDatabaseHelper.ArielTableNames.TABLE_SECURE.equals(tableName)) {
    ...
    private int deleteForUser(int callingUserId, Uri uri, String selection,
        ...
        if (!TextUtils.isEmpty(selection) && selectionArgs.length > 0) {
            ...
            ArielDatabaseHelper dbHelper = getOrEstablishDatabase(getUserIdForTable(tableName,
                    callingUserId));
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        // NOTE: update() is never called by the front-end ArielSettings API, and updates that
    ...
    /**
     * Tries to get a {@link ArielDatabaseHelper} for the specified user and if it does not exist, a
     * new instance of {@link ArielDatabaseHelper} is created for the specified user and returned.
     * @param callingUser
     * @return
     */
    private ArielDatabaseHelper getOrEstablishDatabase(int callingUser) {
    ...
    /**
     * Check if a {@link ArielDatabaseHelper} exists for a user and if it doesn't, a new helper is
    ...
    private void establishDbTracking(int userId) {
        ...
        synchronized (this) {
            ...
            if (LOCAL_LOGV) {
                Log.i(TAG, "Checking ariel settings db helper for user " + userId);
            }
            if (dbHelper == null) {
                if (LOCAL_LOGV) {
                    Log.i(TAG, "Installing new ariel settings db helper for user " + userId);
                }
                dbHelper = new ArielDatabaseHelper(getContext(), userId);
    ...
    private void checkWritePermissions(String tableName) {
        if ((ArielDatabaseHelper.ArielTableNames.TABLE_SECURE.equals(tableName) ||
                ...
                getContext().checkCallingOrSelfPermission(
                        ...
                        PackageManager.PERMISSION_GRANTED) {
            throw new SecurityException(
                    String.format("Permission denial: writing to ariel secure settings requires %1$s",
                        arielos.platform.Manifest.permission.WRITE_SECURE_SETTINGS));
        }
    }
    ...
    private String getTableNameFromUriMatchCode(int code) {
        switch (code) {
            case SYSTEM:
            case SYSTEM_ITEM_NAME:
                return ArielDatabaseHelper.ArielTableNames.TABLE_SYSTEM;
            case SECURE:
            case SECURE_ITEM_NAME:
                return ArielDatabaseHelper.ArielTableNames.TABLE_SECURE;
            case GLOBAL:
            case GLOBAL_ITEM_NAME:
                return ArielDatabaseHelper.ArielTableNames.TABLE_GLOBAL;
            default:
                throw new IllegalArgumentException("Invalid uri match code: " + code);
        }
    }
    ...
    private int getUserIdForTable(String tableName, int userId) {
        return ArielDatabaseHelper.ArielTableNames.TABLE_GLOBAL.equals(tableName) ?
                UserHandle.USER_OWNER : userId;
    }

    /**
     * Modify setting version for an updated table before notifying of change. The
     * {@link ArielSettings} class uses these to provide client-side caches.
     * @param uri to send notifications for
     * @param userId
     */
    private void notifyChange(Uri uri, String tableName, int userId) {
        String property = null;
        final boolean isGlobal = tableName.equals(ArielDatabaseHelper.ArielTableNames.TABLE_GLOBAL);
        if (tableName.equals(ArielDatabaseHelper.ArielTableNames.TABLE_SYSTEM)) {
            property = ArielSettings.System.SYS_PROP_ARIEL_SETTING_VERSION;
        } else if (tableName.equals(ArielDatabaseHelper.ArielTableNames.TABLE_SECURE)) {
            property = ArielSettings.Secure.SYS_PROP_ARIEL_SETTING_VERSION;
        } else if (isGlobal) {
            property = ArielSettings.Global.SYS_PROP_ARIEL_SETTING_VERSION;
        }
    ...
    private void validateSystemSettingNameValue(String name, String value) {
        ArielSettings.Validator validator = ArielSettings.System.VALIDATORS.get(name);
        if (validator == null) {
    ...
    private void validateSecureSettingValue(String name, String value) {
        ArielSettings.Validator validator = ArielSettings.Secure.VALIDATORS.get(name);
