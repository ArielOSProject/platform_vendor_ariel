...
public abstract class ImageClassifier {
    ...
    ImageClassifier() throws IOException {
        tfliteModel = loadModelFromEtc("ariel_nsfw_efficientnet_lite4_full_integer.tflite");
        tflite = new Interpreter(tfliteModel, tfliteOptions);
        labelList = loadLabelListFromEtc("labels.txt");
        imgData =
    ...
    public void classifyImage(Image image, SpannableStringBuilder builder) {
        // first convert image to bitmap
        //Bitmap bitmap = ImageUtils.imageToBitmap(image);
        //classifyFrame(bitmap, builder);
        Log.d(TAG, "inside classifyImage");
    }
    ...
    private MappedByteBuffer loadModelFromEtc(String modelFileName) throws IOException {
        // Define the absolute path to the file in the /etc/models directory
        String modelPath = "/system_ext/etc/models/" + modelFileName;

        // Open the file for reading
        File file = new File(modelPath);
        if (!file.exists()) {
            throw new IOException("Model file does not exist at: " + modelPath);
        }
    ...
    private List<String> loadLabelListFromEtc(String labelFileName) throws IOException {
        // Define the absolute path to the file in the /etc/models directory
        String labelFilePath = "/system_ext/etc/models/" + labelFileName;

        // Open the file for reading
        File file = new File(labelFilePath);
        if (!file.exists()) {
            throw new IOException("Label file does not exist at: " + labelFilePath);
        }
    ...
    private void convertBitmapToByteBuffer(Bitmap bitmap) {
        ...
        imgData.rewind();
        Bitmap resizedBitmap = Bitmap.createScaledBitmap(bitmap, getImageSizeX(), getImageSizeY(), true);
