...
function breakfast()
{
    target=$1
    local variant=$2
    ARIEL_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/ariel/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/ariel-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.ariel.device | grep -q "$ARIEL_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $ARIEL_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    if (adb shell getprop ro.ariel.device | grep -q "$ARIEL_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.ariel.device | grep -q "$ARIEL_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $ARIEL_FIXUP_COMMON_OUT ]; then
